...
`ifdef KRAKEN_FPGA_SYNTH_PROFILE
   // The TCN shift-memory path is the dominant timing hotspot in the reduced
   // FPGA build. For the FPGA-only synthesis profile, keep the last bankset on
   // the regular activation-memory implementation instead of the heavier TCN
   // structure. The functional/simulation profile below still uses the full
   // TCN-specific memory path for behavioral correctness.
   activationmemory
     #(
       .N_I(N_I),
       .N_O(N_O),

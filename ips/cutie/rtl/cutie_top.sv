...
module cutie_top
  import cutie_params::*;
   #(
     ...
     parameter int unsigned OCU_PER_STAGE = N_O/PIPELINEDEPTH,
...
`ifdef KRAKEN_FPGA_SYNTH_PROFILE
   (* max_fanout = 64 *) logic                                                   LUCA_compute_latch_new_layer_o;
`else
...
`ifdef KRAKEN_FPGA_SYNTH_PROFILE
   (* max_fanout = 64 *) logic [0:PIPELINEDEPTH-1]                               ocu_controller_compute_enable_o;
`else
...
`ifdef KRAKEN_FPGA_SYNTH_PROFILE
   (* max_fanout = 64 *) enums_ocu_pool::alu_op ocu_controller_alu_op_o; // 1: Sum, 0: Max
`else
...
`endif
   ...
   generate
      for(pipelinestage=0;pipelinestage<PIPELINEDEPTH;pipelinestage++) begin: pipeline
         ...
         logic [0:OCU_PER_STAGE-1][1:0]                       pipeline_output;
         ///////////////////////////////// PIPELINE STAGE SIGNALS /////////////////////////////////

         if(pipelinestage < PIPELINEDEPTH-1) begin : compute_output_pipeline
            logic [0:PIPELINEDEPTH-2][0:OCU_PER_STAGE-1][1:0] output_pipeline_d, output_pipeline;
         ...
         logic [0:OCU_PER_STAGE-1] weightmemory_controller_rw_collision_i;
         logic [0:OCU_PER_STAGE-1] weightmemory_controller_ready_i; // OCU is ready for inputs
         logic [0:OCU_PER_STAGE-1] weightmemory_controller_valid_i; // Activation memory outputs are valid
         ...
         for(localmodulenum=0;localmodulenum<OCU_PER_STAGE;localmodulenum++) begin: compute_block
            ...
            assign weightmemory_valid_vector[localmodulenum + (N_O/PIPELINEDEPTH)*pipelinestage] =
                weightmemory_ready_o & ~weightmemory_rw_collision_o;
   ...
   // Let the linebuffer reader proceed once the first weight stage has finished
   // loading, even if the one-cycle valid pulse has already passed.
   assign linebuffer_master_controller_ready_i = 1'b1;

...
module evt_engine_router 
	...
	logic        evt_stream_engine_valid_q;
	logic        evt_stream_engine_ready_q;
	uevent_t     evt_stream_engine_evt_q;
	...
	spill_register #(
		.T(uevent_t),
		.Bypass(1'b0)
	) i_evt_engine_input_cut (
		.clk_i  (clk_i),
		.rst_ni (rst_ni),
		.valid_i(evt_stream_engine_src.valid),
		.ready_o(evt_stream_engine_src.ready),
		.data_i (evt_stream_engine_src.evt),
		.valid_o(evt_stream_engine_valid_q),
		.ready_i(evt_stream_engine_ready_q),
		.data_o (evt_stream_engine_evt_q)
	);
...
always_comb begin : proc_decode_input_op
	case (evt_stream_engine_evt_q.spike.operation)
...
always_comb begin : proc_NS
	...
	case (PS)
		...
		RESET         : begin
			if (evt_stream_engine_valid_q) begin
				NS = FETCH_EVT;
			end else begin
				NS = RESET;
			end
...
always_comb begin : proc_outputs
	case (PS)
		...
		RESET         : begin
			...
			evt_stream_engine_ready_q   = 1'b0;
		...
		PROCESS_SPIKE : begin
			...
			evt_stream_engine_ready_q   = 1'b0;
			evt_stream_time_src.evt     = 0;
			evt_stream_spike_src.evt    = evt_stream_engine_evt_q;
		end
		...
		UPDATE_TIME   : begin
			...
			evt_stream_spike_src.valid  = 1'b0;
			evt_stream_engine_ready_q   = 1'b0;
			evt_stream_time_src.evt     = evt_stream_engine_evt_q;
			evt_stream_spike_src.evt    = 0;
		end
		...
		DONE          : begin
			evt_stream_time_src.valid   = 1'b0;
			evt_stream_spike_src.valid  = 1'b0;
			evt_stream_engine_ready_q   = 1'b1;
			evt_stream_time_src.evt     = 0;
			evt_stream_spike_src.evt    = 0;
		end

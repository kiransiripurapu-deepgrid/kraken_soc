...
#( 	parameter DP_GROUP = 16,
	...
	MEMORY_PORT #(.AW(SEQ_ADDR_WIDTH), .DW(STATE_DATA_WIDTH), .CW(1))
		state_read_int [DP_GROUP-1:0] (.clk(engine_clk_i));
	MEMORY_PORT #(.AW(SEQ_ADDR_WIDTH), .DW(STATE_DATA_WIDTH), .CW(1))
		state_write_int[DP_GROUP-1:0] (.clk(engine_clk_i));

	for (genvar i = 0; i < DP_GROUP; i++) begin : gen_state_mem_pipe
		assign state_read_int[i].addr   = state_read[i].addr;
		assign state_read_int[i].enable = state_read[i].enable;
		assign state_read[i].data       = state_read_int[i].data;

		always_ff @(posedge engine_clk_i or negedge engine_rst_ni) begin
			if (!engine_rst_ni) begin
				state_write_int[i].addr   <= '0;
				state_write_int[i].data   <= '0;
				state_write_int[i].enable <= '0;
			end else begin
				state_write_int[i].addr   <= state_write[i].addr;
				state_write_int[i].data   <= state_write[i].data;
				state_write_int[i].enable <= state_write[i].enable;
			end
		end
	end
	...
	evt_status_memory#(
		...
		.test_en_i			(	1'b0				),
		.state_read 		(	state_read_int[DP_GROUP-1:0]),
		.state_write 		(	state_write_int[DP_GROUP-1:0])

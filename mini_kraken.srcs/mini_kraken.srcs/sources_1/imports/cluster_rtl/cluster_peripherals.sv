...
#(
  ...
  parameter int unsigned  NB_TCDM_BANKS         = 4,
...
`endif
  ...
  //********************************************************
  //
  // In this mini_kraken configuration we are not enabling the Snitch I$
  // controller or its performance-counter block. The original PULP code
  // would instantiate additional control logic here; that environment is not
  // present in this extracted repo, so we omit it to keep synthesis clean.
  ...
  generate
  if (NB_CACHE_BANKS > 1) begin: gen_icache_ctrl

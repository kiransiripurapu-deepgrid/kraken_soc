 ...
 */
`define DEFAULT_BW 64
`define DEFAULT_UW 0
`define DEFAULT_IW 0
`define DEFAULT_EW 0
`define DEFAULT_EHW 0
...
#(
  ...
  localparam int unsigned NB_CORES = 2,
...
// Replace the positional version with this explicit mapping
...
// cores -> HCI TCDM (to cluster interconnect)
hci_core_intf #(
  .DW ( HciCoreSizeParam.DW ),
  .AW ( HciCoreSizeParam.AW )
...
core_data_rsp_t [NB_CORES:0] core_data_rsp, demux_data_rsp;
...
rapid_recovery_t [NB_CORES-1:0] recovery_bus;

logic [1:0] clk_core;
logic [1:0] setback;
logic [1:0][4:0] ext_perf;
...
generate
  for (genvar i = 0; i < Cfg.NumCores; i++) begin : CORE
    ...
    core_demux_wrap        #(
      .AddrWidth           ( AddrWidth              ),
      .DataWidth           ( DataWidth              ),
      .RemapAddress        ( 1'b0                   ), // Replaced Cfg.EnableRemapAddress
      .ClustAlias          ( 12'h000                ), // Replaced Cfg.ClusterAlias
      .ClustAliasBase      ( 12'h000                ), // Replaced Cfg.ClusterAliasBase
      .NumExtPerf          ( 5                      ),
      .core_data_req_t     ( core_data_req_t        ),
      .core_data_rsp_t     ( core_data_rsp_t        )
    ) i_core_demux         (
      .clk_i               ( clk_core[i]            ),
      // ... rest of the ports remain the same
      .rst_ni              ( rst_ni                ),
      .test_en_i           ( test_mode_i           ),
...
// =========================================================================
// --- LEAN BUILD BYPASS: HMR UNIT REMOVED ---
// =========================================================================
generate
  begin : gen_no_hmr_unit
...
if (Cfg.SnitchICache) begin : gen_snitch_icache
 ...
 PRI_ICACHE_CTRL_UNIT_BUS IC_ctrl_unit_bus_pri_intf [1:0] ();
  icache_hier_top #(
    ...
    // If the port expects a 2-element array of interfaces
    // Pass the array directly to the port
    .IC_ctrl_unit_bus_pri ( IC_ctrl_unit_bus_pri_intf ),
    .IC_ctrl_unit_bus_main  ( IC_ctrl_unit_bus_main     )
  );
end

...
`ifdef SYNTHESIS
  // Functional-synthesis stub: no real TCDM SRAM/ECC in this build yet.
  generate
    for (genvar i = 0; i < NbBanks; i++) begin : banks_gen_stub
      assign scrub_fix_o[i]           = 1'b0;
      assign scrub_uncorrectable_o[i] = 1'b0;
      assign ecc_single_error_o[i]    = 1'b0;
      assign ecc_multiple_error_o[i]  = 1'b0;

      assign tcdm_slave[i].gnt     = 1'b1;
      assign tcdm_slave[i].r_valid = tcdm_slave[i].req;
      assign tcdm_slave[i].r_data  = '0;
      assign tcdm_slave[i].r_user  = '0;
      assign tcdm_slave[i].r_id    = '0;
      assign tcdm_slave[i].r_opc   = '0;

      // ECC sideband (present when ECC interconnect is enabled upstream).
      assign tcdm_slave[i].egnt     = 1'b1;
      assign tcdm_slave[i].r_evalid = 1'b0;
